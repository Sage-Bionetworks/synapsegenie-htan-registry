...
#        return filepath
    ...
    def _validate(self, entity):
        ...
        if showinf_exc.returncode:
            total_error.append(f"ERROR: showinf failed")
            warning.append(showinf_exc.stdout)

        # Validate the OME-XML with xmlvalid if file type is ome.tiff
        if os.path.basename(path_or_df).endswith(("ome.tif",".ome.tiff")):
            ...
            if xmlvalid_exc.returncode:
                ...
                warning.append(xmlvalid_exc.stdout)
        ...
        if tiffinfo_exc.returncode:
            ...
            warning.append(tiffinfo_exc.stderr)
